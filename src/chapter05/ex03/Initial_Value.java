...
public class Initial_Value {
	...
	public static void main(String[] args) {
		...
		//	참조 자료형 : 배열, 열거형, 인터페이스형, ...
		// 			   강제 초기화 가능. boolean(false), 정수(0), 실수(0.0), 문자(공백), 문자열(null)
		//			 ㄴ배열: stack 공간에 [배열] 변수와 Heap영역의 주소 정보가 저장
		//					Heap 영역에는 값이 저장
		// 1. stack 메모리값 (강제 초기화 되지 않음)
		int value1;
		// System.out.println(value1); 오류 발생: 초기 값이 없음
		int [] value2;
		...
		int[]value4 = null ; 		//null : 비어있는 상태, 참조 자료형에서만 사용 가능
		System.out.println(value4); //value4에 Heap영역의 주소가 비어있는 상태 : null

		System.out.println("===================");
		//2.Heap영역은 강제 초기화된다. boolean(false), 정수(0), 실수(0.0)
		//2-1. 기본자료형 배열
		
		boolean[] arr1 = new boolean[3]; //기본 값으로 false가 각 방에 할당됨
		...
		System.out.println();
		System.out.println("=========문자 : 초기값으로 <<출력안됨>>==========");
